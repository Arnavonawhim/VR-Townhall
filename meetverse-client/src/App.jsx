...
.nav-ghost { background: transparent; color: var(--dim); border: 1px solid var(--border); }
.nav-ghost:hover { color: var(--white); border-color: var(--mid); }
...
.hero-eyebrow { display: flex; align-items: center; gap: 20px; margin-bottom: 48px; }
.hero-tag { font-size: 0.7rem; letter-spacing: 0.14em; text-transform: uppercase; color: var(--dim); font-weight: 500; }
.hero-tag-accent { color: var(--accent); }
.rule-h { width: 40px; height: 1px; background: var(--border); }
.hero-body { display: grid; grid-template-columns: 1fr 1fr; gap: 80px; align-items: start; padding-bottom: 80px; flex: 1; }
.hero-title {
  font-family: 'Bebas Neue', sans-serif;
  font-size: clamp(72px, 9vw, 136px);
  line-height: 0.9; letter-spacing: 0.01em; color: var(--white); margin-bottom: 32px;
}
.hero-title .t-accent { color: var(--accent); }
.hero-title .t-italic { font-family: 'Instrument Serif', serif; font-style: italic; font-size: 0.88em; color: var(--off-white); }
.hero-desc { font-size: 1rem; color: var(--dim); line-height: 1.8; max-width: 400px; margin-bottom: 44px; }
...
.vis-ring::after {
  ...
  background: rgba(212,82,42,0.1); filter: blur(14px);
}
.vis-dot { position: absolute; width: 5px; height: 5px; border-radius: 50%; background: var(--accent); z-index: 2; }
.vd1 { top: 16px; left: 50%; } .vd2 { bottom: 24px; right: 28%; } .vd3 { left: 20px; top: 44%; }
@keyframes vspin { to { transform: rotate(360deg); } }
.vis-label { position: absolute; bottom: 14px; left: 16px; font-size: 0.65rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); }
.vis-label span { color: var(--accent); }
.hero-stats { display: grid; grid-template-columns: repeat(3, 1fr); border: 1px solid var(--border); border-radius: 4px; overflow: hidden; }
.h-stat { padding: 18px 20px; border-right: 1px solid var(--border); }
.h-stat:last-child { border-right: none; }
.h-stat-num { font-family: 'Bebas Neue', sans-serif; font-size: 1.8rem; color: var(--white); letter-spacing: 0.04em; line-height: 1; }
.h-stat-num .a { color: var(--accent); }
.h-stat-lbl { font-size: 0.68rem; letter-spacing: 0.08em; text-transform: uppercase; color: var(--muted); margin-top: 4px; line-height: 1.4; }

/* MARQUEE */
.marquee-wrap { border-top: 1px solid var(--border); border-bottom: 1px solid var(--border); padding: 13px 0; overflow: hidden; background: var(--off-black); }
.marquee-inner { display: flex; gap: 56px; width: max-content; animation: mq 28s linear infinite; }
.mq-item { font-family: 'Bebas Neue', sans-serif; font-size: 1rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); white-space: nowrap; display: flex; align-items: center; gap: 56px; }
...
.sec-header { display: grid; grid-template-columns: 260px 1fr; gap: 48px; margin-bottom: 64px; align-items: start; }
.sec-lbl { font-size: 0.7rem; letter-spacing: 0.14em; text-transform: uppercase; color: var(--dim); padding-top: 6px; font-weight: 500; }
.sec-num { font-family: 'Bebas Neue', sans-serif; font-size: 0.85rem; color: var(--accent); letter-spacing: 0.1em; margin-bottom: 6px; }
.sec-title { font-family: 'Bebas Neue', sans-serif; font-size: clamp(40px, 5vw, 68px); line-height: 0.93; letter-spacing: 0.02em; color: var(--white); margin-bottom: 14px; }
...
.how-item { display: grid; grid-template-columns: 80px 1fr 160px; gap: 32px; align-items: center; padding: 26px 0; border-bottom: 1px solid var(--border); transition: background 0.12s; cursor: default; }
.how-item:first-child { border-top: 1px solid var(--border); }
.how-item:hover { background: rgba(255,255,255,0.015); }
.how-n { font-family: 'Bebas Neue', sans-serif; font-size: 1.7rem; color: var(--border); letter-spacing: 0.04em; transition: color 0.15s; }
.how-item:hover .how-n { color: var(--accent); }
...
.how-tag { font-size: 0.65rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); white-space: nowrap; text-align: right; font-weight: 500; }

/* STICKY NOTES */
.sticky-section { padding: 96px 48px; border-bottom: 1px solid var(--border); }
.sticky-board { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }
.sticky { padding: 28px 26px 24px; border-radius: 2px; position: relative; transition: transform 0.2s, box-shadow 0.2s; cursor: default; min-height: 210px; display: flex; flex-direction: column; justify-content: space-between; }
.sticky:hover { transform: translateY(-4px) rotate(0deg); box-shadow: 0 16px 40px rgba(0,0,0,0.5); z-index: 2; }
...
.sticky::before { content: ''; position: absolute; top: 0; left: 50%; transform: translateX(-50%); width: 40px; height: 6px; border-radius: 0 0 3px 3px; background: rgba(0,0,0,0.15); }
.sticky-cat { font-size: 0.6rem; letter-spacing: 0.14em; text-transform: uppercase; font-weight: 700; opacity: 0.5; margin-bottom: 10px; font-family: 'Instrument Sans', sans-serif; }
.sticky-title { font-family: 'Bebas Neue', sans-serif; font-size: 1.5rem; letter-spacing: 0.02em; line-height: 1; margin-bottom: 10px; opacity: 0.9; }
.sticky-desc { font-size: 0.82rem; line-height: 1.6; opacity: 0.7; font-family: 'Instrument Sans', sans-serif; flex: 1; }
.sticky-pills { display: flex; flex-wrap: wrap; gap: 5px; margin-top: 14px; }
.sticky-pill { padding: 3px 7px; border-radius: 2px; font-size: 0.6rem; letter-spacing: 0.07em; text-transform: uppercase; font-weight: 600; background: rgba(0,0,0,0.1); font-family: 'Instrument Sans', sans-serif; }

/* ROLES */
.roles-grid { display: grid; grid-template-columns: repeat(3, 1fr); border: 1px solid var(--border); border-radius: 4px; overflow: hidden; }
...
.rm-accent { background: var(--accent); } .rm-white { background: var(--white); } .rm-mid { background: var(--mid); }
.role-lbl { font-size: 0.65rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--dim); margin-bottom: 8px; font-weight: 500; }
.role-title { font-family: 'Bebas Neue', sans-serif; font-size: 1.9rem; letter-spacing: 0.02em; color: var(--white); margin-bottom: 14px; line-height: 1; }
...
.role-perks li { font-size: 0.8rem; color: var(--muted); display: flex; gap: 10px; align-items: flex-start; line-height: 1.5; }
.perk-dash { color: var(--accent); flex-shrink: 0; }

/* CTA */
.cta-block { padding: 100px 48px; display: grid; grid-template-columns: 1fr 1fr; gap: 80px; align-items: center; border-bottom: 1px solid var(--border); }
.cta-title { font-family: 'Bebas Neue', sans-serif; font-size: clamp(52px, 6.5vw, 100px); line-height: 0.9; letter-spacing: 0.02em; color: var(--white); }
...
/* FOOTER */
.footer { padding: 28px 48px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px; }
...
.modal-x { position: absolute; top: 18px; right: 18px; width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; background: none; border: 1px solid var(--border); color: var(--muted); cursor: pointer; border-radius: 3px; font-size: 0.8rem; transition: all 0.15s; }
.modal-x:hover { border-color: var(--mid); color: var(--white); }
.modal-eyebrow { font-size: 0.65rem; letter-spacing: 0.14em; text-transform: uppercase; color: var(--accent); font-weight: 600; margin-bottom: 8px; }
.modal-title { font-family: 'Bebas Neue', sans-serif; font-size: 2.6rem; letter-spacing: 0.02em; color: var(--white); margin-bottom: 4px; line-height: 1; }
.modal-sub { font-size: 0.85rem; color: var(--dim); margin-bottom: 32px; line-height: 1.6; }
.fgroup { margin-bottom: 14px; }
.flabel { display: block; font-size: 0.65rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); margin-bottom: 6px; font-weight: 600; }
.finput { width: 100%; padding: 10px 13px; background: var(--card); border: 1px solid var(--border); color: var(--white); font-family: 'Instrument Sans', sans-serif; font-size: 0.88rem; border-radius: 3px; outline: none; transition: border 0.15s; }
.finput:focus { border-color: var(--muted); }
.finput::placeholder { color: var(--muted); }
.role-row { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 14px; }
.role-opt { padding: 10px; border: 1px solid var(--border); border-radius: 3px; background: transparent; color: var(--dim); font-family: 'Instrument Sans', sans-serif; font-size: 0.8rem; cursor: pointer; transition: all 0.15s; text-align: center; letter-spacing: 0.02em; }
...
.sbtn { width: 100%; padding: 12px; background: var(--white); color: var(--black); border: none; border-radius: 3px; font-family: 'Instrument Sans', sans-serif; font-size: 0.78rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; cursor: pointer; transition: background 0.15s; margin-top: 6px; display: flex; align-items: center; justify-content: center; gap: 8px; }
...
.mswitch button { background: none; border: none; color: var(--white); cursor: pointer; font-family: 'Instrument Sans', sans-serif; font-size: 0.8rem; text-decoration: underline; text-underline-offset: 3px; }
.merr { background: rgba(212,82,42,0.1); border: 1px solid rgba(212,82,42,0.3); border-radius: 3px; padding: 9px 13px; color: #e0896e; font-size: 0.82rem; margin-bottom: 14px; }
.mok { background: rgba(80,180,120,0.1); border: 1px solid rgba(80,180,120,0.3); border-radius: 3px; padding: 9px 13px; color: #8dd4a8; font-size: 0.82rem; margin-bottom: 14px; }
.spin { width: 13px; height: 13px; border-radius: 50%; border: 2px solid rgba(0,0,0,0.15); border-top-color: var(--black); animation: sp 0.6s linear infinite; }
@keyframes sp { to { transform: rotate(360deg); } }

/* DASHBOARD */
.dash { min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 100px 48px; }
.dash-card { border: 1px solid var(--border); border-radius: 4px; max-width: 500px; width: 100%; overflow: hidden; }
.dash-head { padding: 36px 40px; border-bottom: 1px solid var(--border); background: var(--card); }
.dash-role { font-size: 0.65rem; letter-spacing: 0.14em; text-transform: uppercase; color: var(--accent); font-weight: 600; margin-bottom: 8px; }
.dash-name { font-family: 'Bebas Neue', sans-serif; font-size: 2.4rem; letter-spacing: 0.02em; color: var(--white); line-height: 1; }
.dash-body { padding: 28px 40px; }
.dash-status-box { background: var(--card); border: 1px solid var(--border); border-radius: 3px; padding: 14px; margin-bottom: 20px; }
...
.dash-info-row { display: flex; flex-direction: column; gap: 8px; margin-bottom: 20px; }
.dash-info-item { display: flex; justify-content: space-between; align-items: center; padding: 10px 14px; background: var(--card); border: 1px solid var(--border); border-radius: 3px; }
.dash-info-key { font-size: 0.65rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); font-weight: 600; }
.dash-info-val { font-size: 0.82rem; color: var(--white); }
.dash-desc { font-size: 0.875rem; color: var(--dim); line-height: 1.75; margin-bottom: 20px; }
.btn-unity { width: 100%; padding: 13px; background: var(--accent); color: var(--white); border: none; border-radius: 3px; font-family: 'Instrument Sans', sans-serif; font-size: 0.78rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; cursor: pointer; transition: background 0.15s; margin-bottom: 10px; display: flex; align-items: center; justify-content: center; gap: 8px; }
.btn-unity:hover { background: #bf4924; }
.btn-out { background: none; border: 1px solid var(--border); color: var(--muted); padding: 8px 18px; border-radius: 3px; cursor: pointer; font-family: 'Instrument Sans', sans-serif; font-size: 0.75rem; letter-spacing: 0.07em; text-transform: uppercase; transition: all 0.15s; }
...
// ─── NAV ─────────────────────────────────────────────────────────────────────
function Nav({ user, onLogin, onSignup, onLogout }) {
  return (
    <nav className="nav">
      <div className="nav-logo" onClick={() => window.scrollTo(0, 0)}>Nexa<span>Verse</span></div>
      ...
      <div className="nav-actions">
        {user ? (
          <>
            <span style={{ fontSize: "0.75rem", color: "var(--dim)", letterSpacing: "0.06em", textTransform: "uppercase" }}>
              {user.name}
            </span>
            <button className="nav-btn nav-ghost" onClick={onLogout}>Sign Out</button>
          </>
        ) : (
          <>
  ...
  return (
    <section className="hero">
      <div className="hero-eyebrow">
        <span className="hero-tag hero-tag-accent">India Gov Hackathon 2026</span>
      ...
      <div className="hero-body">
        <div>
          ...
          <p className="hero-desc">
            A virtual presence platform for government officials, educators, and institutions.
            AI avatars, immersive sessions, and VR compatibility — all on a secure, scalable infrastructure.
          </p>
    ...
    <section className="section" id="roles">
      <div className="sec-header">
        ...
        <div>
          ...
          <h2 className="sec-title">Who uses<br /><span className="ital">NexaVerse</span></h2>
        </div>
      </div>
...
function Footer() {
  return (
    <footer className="footer">
      <div className="footer-logo">Nexa<span>Verse</span></div>
      <div className="footer-copy">Built for India Gov Hackathon 2026</div>
    </footer>
  );
}
...
function LoginModal({ onClose, onSwitch, onSuccess }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (data.success) {
        // Save JWT token — Unity WebGL build will read this from localStorage
  ...
  return (
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div className="modal-eyebrow">NexaVerse Platform</div>
...
function SignupModal({ onClose, onSwitch, onSuccess }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (data.success) {
        // Save JWT + user — DB record is created server-side by this call
        localStorage.setItem("nv_token", data.token);
        localStorage.setItem("nv_user", JSON.stringify(data.user));
      ...
      <div className="modal">
        ...
        <div className="fgroup">
          ...
          <div className="role-row">
            <button className={`role-opt ${f.userType === "viewer" ? "on" : ""}`} onClick={() => set("userType", "viewer")}>
              Viewer / Student
            </button>
            <button className={`role-opt ${f.userType === "creator" ? "on" : ""}`} onClick={() => set("userType", "creator")}>
              Creator / Leader
            </button>
          </div>
        </div>
        ...
        <div className="fgroup">
          <label className="flabel">Password</label>
          <input className="finput" type="password" placeholder="Min. 8 characters"
            value={f.password} onChange={e => set("password", e.target.value)} />
        </div>
        {f.userType === "creator" && (
          <div className="fgroup">
            <label className="flabel">Organisation Name</label>
            <input className="finput" type="text" placeholder="e.g. District Administration"
              value={f.organizationName} onChange={e => set("organizationName", e.target.value)} />
          </div>
        )}
        <button className="sbtn" onClick={submit} disabled={loading}>
...
function Dashboard({ user, onLogout }) {
  ...
  // When you have your Unity WebGL build hosted, update this URL.
  // The token + userId are passed as query params so Unity can read them
  // via Application.absoluteURL and then call GET /api/v1/auth/me to get
  // the full user + avatar data.
  function launchUnity() {
    const unityUrl = `/unity/index.html?token=${token}&userId=${user.id}&userType=${user.userType}`;
      ...
      <div className="dash-card">
        ...
        <div className="dash-body">
          <div className="dash-status-box">
            <div className="dash-slbl">Session Status</div>
            <div className="dash-sval">
              <span className="sdot" />Authenticated — Ready to enter NexaVerse
            </div>
          </div>

          <div className="dash-info-row">
            <div className="dash-info-item">
              <span className="dash-info-key">Email</span>
              <span className="dash-info-val">{user?.email}</span>
            </div>
            <div className="dash-info-item">
              <span className="dash-info-key">Role</span>
              <span className="dash-info-val" style={{ textTransform: "capitalize" }}>{user?.userType}</span>
            </div>
            {user?.organizationName && (
              <div className="dash-info-item">
                <span className="dash-info-key">Organisation</span>
                <span className="dash-info-val">{user.organizationName}</span>
              </div>
            )}
            <div className="dash-info-item">
              <span className="dash-info-key">User ID</span>
              <span className="dash-info-val" style={{ fontSize: "0.7rem", color: "var(--muted)", fontFamily: "monospace" }}>
                {user?.id}
              </span>
            </div>
          </div>
          ...
          <p className="dash-desc">
            {user?.userType === "creator"
              ? "Your session is ready. Launch the Unity build to customise your avatar and host a live session."
              : "Your account is active. Launch the Unity build to join a session with your avatar."}
          </p>

          <button className="btn-unity" onClick={launchUnity}>Enter NexaVerse →</button>
...
export default function App() {
  ...
  const [user, setUser] = useState(() => {
    try { return JSON.parse(localStorage.getItem("nv_user")); }
    catch { return null; }
  });
  ...
  return (
    <>
      <StyleInjector />
      <Nav user={user} onLogin={() => setModal("login")} onSignup={() => setModal("signup")} onLogout={onLogout} />
